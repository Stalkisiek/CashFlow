...
export const useSavingsApi = () => {
    ...
    const fetchData = async() => {
        try{
            ...
            const hasCorrectType = serviceResponse.data.some((bankAccount) => bankAccount.type === 1);
            if(!hasCorrectType){
                navigate('/home');
                throw new Error('You dont have this account');
            }

            const bankAccount:BankAccount|undefined = serviceResponse.data.find((bankAccount) => bankAccount.type === 1);
